-- Enriquecimento de contexto para a tela de conferência sem mover regra para o frontend.
-- A view continua como fonte única da UI e adiciona apenas campos informativos da Base.
...
SELECT
  ...
  -- Leitura dinâmica de JSON por coluna configurada: se chave não existir/mapeamento faltar,
  -- retorna NULL (frontend converte para "—") para evitar quebra silenciosa da conferência.
  NULLIF(TRIM(CASE
